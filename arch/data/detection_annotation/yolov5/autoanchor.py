...
def check_anchor_order(anchors, stride):
    # Check anchor order against stride order for YOLOv5 Detect() module m, and correct if necessary
    a = anchors.prod(-1).mean(-1).view(-1)  # mean anchor area per output layer
    da = a[-1] - a[0]  # delta a
...
def kmean_anchors(dataset, n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    def print_results(k, verbose=True):
        ...
        s = (
            f"thr={thr:.2f}: {bpr:.4f} best possible recall, {aat:.2f} anchors past thr\n"
    ...
    try:
        ...
        logger.warning("WARNING: switching strategies from kmeans to random init")
...
def check_anchors(dataset, anchors, strides, thr=4.0, imgsz=640):
    assert isinstance(anchors, torch.Tensor) and isinstance(strides, torch.Tensor)
    assert anchors.device == strides.device
    ...
    if bpr > 0.98:  # threshold to recompute
        ...
        logger.success(s)
    return anchors.half().float()  # pre-reduce anchor precision
