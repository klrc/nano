...
def sync_settings(config: SampleSettings):
    if config is None:
        return SampleSettings()
    assert isinstance(config, SampleSettings)
    return config


def sync_status(train_loader, settings: SampleSettings):
    s, u = settings, Status()
...
def sync_yolov5_hyp(model, settings: SampleSettings):
    s = settings
...
def check_forward(model: nn.Module, device, input_shape, expected_output_shapes=None):
...
def create_dataloader(dataset_path, training, settings: SampleSettings):
    s = settings
...
def multi_task_backward(
    model: nn.Module, device, tasks, optimizer, scaler, ema, scheduler, lf, settings: SampleSettings, status: Status, fast_debug_mode=False
):
    s = settings
    u = status
    ...
    # get main task
    main_task = [t for t in tasks if t["is_main_task"]][0]
    pbar = range(len(main_task["train_loader"]))
    ...
    for i in pbar:  # batch -------------------------------------------------------------
        ...
        for ti, task in enumerate(tasks):
            task_names = task["settings"].names
            train_loader = task["train_loader"]
            task_ports = task["ports"]
            criteria = task["criteria"]
            task_weight = task["weight"]
            train_loader: InfiniteDataLoader
            imgs, targets, paths, _ = next(train_loader.iterator)
            imgs = imgs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0

            # plot image
            if i == 0 and s.save_plot:
                plot_images(imgs, targets, paths, f"{u.save_dir}/train_batch_0_task_{ti}.jpg", task_names)

            # Forward
            with amp.autocast(enabled=cuda):
                pred = model(imgs)  # forward
                pred = pred[..., task_ports]
                task_loss, task_loss_items = criteria(pred, targets.to(device))  # loss scaled by batch_size
                loss += task_loss * task_weight
                loss_items += task_loss_items * task_weight
...
@torch.no_grad()
def val_for_one_epoch(model, task_ports, device, val_loader, criteria, settings: SampleSettings, status: Status, fast_debug_mode=False, is_main_task=False):
    s, u = settings, status
    cuda = device.type != "cpu"
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        out = out[..., task_ports]
        train_out = train_out[..., task_ports]
...
def save_data(data, path, mode="w"):
    if isinstance(data, SampleSettings) or isinstance(data, Status):
        assert mode in ("w", "a")
