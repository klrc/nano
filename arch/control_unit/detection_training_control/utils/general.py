...
def train_for_one_epoch(
    model: nn.Module, device, train_loader, optimizer, criteria, scaler, ema, scheduler, lf, settings: SampleSettings, status: Status, fast_debug_mode=False
):
    s = settings
    u = status
    # training pipeline
...
@torch.no_grad()
def val_for_one_epoch(model, device, val_loader, criteria, settings: SampleSettings, status: Status, fast_debug_mode=False):
    s, u = settings, status
    cuda = device.type != "cpu"
