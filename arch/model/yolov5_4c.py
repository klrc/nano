...
from .detection_utils import BBv5, DH4C
...
class Yolov5(nn.Module):
    def __init__(self, num_classes=80, width_multiple=0.25, depth_multiple=0.33) -> None:
        ...
        self.head = DH4C(self.backbone.out_channels, num_classes, strides=self.backbone.strides)
    ...
    def summary(self, tsize=(4, 3, 416, 416)):
        ...
        params /= 1e6
        flops /= 1e9
        logger.success(f"Yolov5(4C) Summary @Resolution = {tsize[2:]}")
                ...
                if pretrained:
                    for pattern in name_dict:
                        if pattern in k:
                            ...
                            # transfer weights from 41c to 4c
                            if "head.m." in k:
                                rm_index = [i * (self.head.nc + 5) + 4 for i in range(self.head.na)]
                                rm_index = [i for i in range(v.shape[0]) if i not in rm_index]
                                v = v[rm_index]
                            break
                    else:
                        print(k)
...
def yolov5n_4c(num_classes=80, weight=None, pretrained=False):
...
def yolov5s_4c(num_classes=80, weight=None, pretrained=False):
...
def yolov5m_4c(num_classes=80, weight=None, pretrained=False):
