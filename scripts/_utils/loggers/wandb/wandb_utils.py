...
from _utils.datasets import LoadImagesAndLabels
from _utils.datasets import img2label_paths
from _utils.general import check_dataset, check_file
...
class WandbLogger():
       ...
       """
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            ...
            self.wandb_run = wandb.init(config=opt,
                                        resume="allow",
                                        project='Nano' if opt.project == 'runs/train' else Path(opt.project).stem,
                                        entity=opt.entity,
    ...
    def check_and_upload_dataset(self, opt):
        ...
        config_path = self.log_dataset_artifact(check_file(opt.data),
                                                opt.single_cls,
                                                'Nano' if opt.project == 'runs/train' else Path(opt.project).stem)
