...
def export(cocox, root):
    ...
    coco_s_cid = {name: i for i, name in enumerate(cocox.target_classes)}
    # true_cid = dict()
        ...
        for m in tqdm(cocox.data[dataset]):
            ...
            with open(target_path, 'w') as f:
                for bbox in m.annotations:
                    line = (coco_s_cid[bbox.name], bbox.x, bbox.y, bbox.w, bbox.h)
...
def prune_func(cocox, __dist, dataset='train2017'):
    ...
    for m in tqdm(cocox.data[dataset]):
        ...
        drop_rate = 0.8
        if len(anno_ark) > 0 and person_ins / len(anno_ark) > 0.5:
            if random.random() < drop_rate:
                is_valid = False

        # replace raw annotations
        m.annotations = anno_ark
        if is_valid:
            ark.append(m)
            for b in anno_ark:
                if b.name not in classes:
                    classes.append(b.name)
